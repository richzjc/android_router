...
public class MUtils {
    ...
    public static M3U8 parseIndex(String url) throws IOException {
        ...
        while ((line = reader.readLine()) != null) {
            ...
            URL lengthUrl = new URL(M3U8Ts.obtainFullUrl(line, basepath));
            HttpURLConnection conn = (HttpURLConnection) lengthUrl.openConnection();
            conn.setConnectTimeout(10 * 1000);
            conn.setReadTimeout(30 * 60 * 1000);
            M3U8Ts m3U8Ts = new M3U8Ts(line, seconds);
            if (conn.getResponseCode() == 200) {
                m3U8Ts.setFileSize(conn.getContentLength());
            }
            ret.addTs(m3U8Ts);
            seconds = 0;
        }
    ...
    public static void deleteM3u8(File file) throws IOException {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.endsWith("ts")) {
                    try {
                        File tempFile = new File(file.getParentFile().getAbsolutePath(), line);
                        tempFile.delete();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
